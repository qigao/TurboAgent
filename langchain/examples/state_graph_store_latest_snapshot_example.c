...
typedef struct {
  const char *text;
} state_graph_store_example_payload_t;

static const char *state_graph_store_example_text(const char *text) {
  return text ? text : "(null)";
}

static turbo_runtime_data_bind_value_t *state_graph_store_example_make_string_array(
    const char *value) {
...
static int state_graph_store_example_start_node(turbo_state_graph_exec_ctx_t *ctx,
                                                void *user_data) {
  const state_graph_store_example_payload_t *payload =
      (const state_graph_store_example_payload_t *)user_data;
  ...
  if (turbo_runtime_data_bind_object_set(
      ...
      turbo_runtime_data_bind_object_set(
          ctx->update, "messages",
          state_graph_store_example_make_string_array(payload->text)) !=
          TURBO_RUNTIME_DATA_BIND_OK) {
    return -1;
  }
  return 0;
...
static int state_graph_store_example_done_node(turbo_state_graph_exec_ctx_t *ctx,
                                               void *user_data) {
  (void)user_data;
  ...
  if (turbo_runtime_data_bind_object_set(
          ...
          TURBO_RUNTIME_DATA_BIND_OK ||
      turbo_runtime_data_bind_object_set(
          ctx->update, "messages", state_graph_store_example_make_string_array("done")) !=
          TURBO_RUNTIME_DATA_BIND_OK) {
    return -1;
  }
  return 0;
...
static turbo_state_graph_t *state_graph_store_example_create_graph(
    const state_graph_store_example_payload_t *payload) {
  turbo_state_graph_t *graph = turbo_state_graph_create("state-graph-store-example");
  ...
  if (turbo_state_graph_add_bind_node(graph, "start", state_graph_store_example_start_node,
                                      (void *)payload) != TURBO_STATE_GRAPH_OK ||
      turbo_state_graph_add_bind_node(graph, "done", state_graph_store_example_done_node, NULL) !=
...
static int state_graph_store_example_print_thread_view(turbo_state_graph_t *graph,
  ...
  if (turbo_state_graph_get_latest_run(graph, thread_id, &latest_run) != TURBO_STATE_GRAPH_OK ||
      turbo_state_graph_get_thread_state(graph, thread_id, &thread_state) != TURBO_STATE_GRAPH_OK) {
  ...
  printf(
      ...
      "  has_pending_run: %s\n",
      label,
      state_graph_store_example_text(turbo_runtime_data_bind_value_as_string(
...
int main(void) {
  state_graph_store_example_payload_t payload = {"review"};
  ...
  graph = state_graph_store_example_create_graph(&payload);
  restored = state_graph_store_example_create_graph(&payload);
  store = turbo_state_graph_store_memory_create();
  patch = turbo_runtime_data_bind_value_create_object();
  if (!graph || !restored || !store.user_data || !patch) {
    fprintf(stderr, "failed to initialize state graph store example\n");
    goto cleanup;
  }

  options.interrupt_before_nodes = interrupt_before;
  options.interrupt_before_count = 1;
  if (turbo_state_graph_start(graph, "thread-example", NULL, &options, &interrupted, &state) !=
      TURBO_STATE_GRAPH_INTERRUPTED) {
    fprintf(stderr, "expected initial run to interrupt before done\n");
    goto cleanup;
  }
  turbo_runtime_data_bind_value_destroy(state);
  state = NULL;

  if (turbo_state_graph_store_save_snapshot(&store, "snapshot-1", graph) !=
      TURBO_STATE_GRAPH_OK) {
    fprintf(stderr, "save snapshot-1 failed\n");
    goto cleanup;
  }
  ...
  if (turbo_runtime_data_bind_object_set(
      ...
      turbo_runtime_data_bind_object_set(
          ...
          TURBO_RUNTIME_DATA_BIND_OK) {
    fprintf(stderr, "failed to build patch\n");
    goto cleanup;
  }

  if (turbo_state_graph_update_state(graph, interrupted.run_id, patch, NULL, &updated, &state) !=
      TURBO_STATE_GRAPH_OK) {
    fprintf(stderr, "update_state failed\n");
    goto cleanup;
  }
  turbo_runtime_data_bind_value_destroy(state);
  state = NULL;

  if (turbo_state_graph_store_save_snapshot(&store, "snapshot-2", graph) !=
      TURBO_STATE_GRAPH_OK) {
    fprintf(stderr, "save snapshot-2 failed\n");
    goto cleanup;
  }
  ...
  if (turbo_state_graph_store_save_snapshot(&store, "snapshot-3", graph) !=
      TURBO_STATE_GRAPH_OK) {
    fprintf(stderr, "save snapshot-3 failed\n");
    goto cleanup;
  }

  if (turbo_state_graph_store_get_latest_snapshot_id_for_thread(
          &store, "thread-example", &latest_snapshot_id) != 0) {
    fprintf(stderr, "get_latest_snapshot_id_for_thread failed\n");
    goto cleanup;
  }
  printf("latest snapshot for thread-example: %s\n", latest_snapshot_id);
  ...
  if (turbo_state_graph_store_get_latest_snapshot_id(&store, &filter, &latest_snapshot_id) != 0) {
    fprintf(stderr, "get_latest_snapshot_id(interrupted) failed\n");
    goto cleanup;
  }
  printf("latest interrupted snapshot: %s\n", latest_snapshot_id);
  free(latest_snapshot_id);
  latest_snapshot_id = NULL;

  if (turbo_state_graph_store_load_latest_snapshot(&store, &filter, restored) !=
      TURBO_STATE_GRAPH_OK) {
    fprintf(stderr, "load_latest_snapshot(interrupted) failed\n");
    goto cleanup;
  }
  if (state_graph_store_example_print_thread_view(restored, "thread-example",
                                                  "restored interrupted snapshot") != 0) {
    fprintf(stderr, "print interrupted snapshot failed\n");
    goto cleanup;
  }

  if (turbo_state_graph_store_load_latest_snapshot_for_thread(
          &store, "thread-example", restored) != TURBO_STATE_GRAPH_OK) {
    fprintf(stderr, "load_latest_snapshot_for_thread failed\n");
    goto cleanup;
  }
  if (state_graph_store_example_print_thread_view(restored, "thread-example",
                                                  "restored latest thread snapshot") != 0) {
    fprintf(stderr, "print latest thread snapshot failed\n");
    goto cleanup;
  }
