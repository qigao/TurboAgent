#ifndef TURBO_AGENT_REMOTE_APP_H
#define TURBO_AGENT_REMOTE_APP_H
...
typedef struct turbo_agent_remote_app_s turbo_agent_remote_app_t;

typedef struct turbo_agent_remote_app_config_s {
  const turbo_agent_remote_session_config_t *session_config;
  const char *graph_name;
} turbo_agent_remote_app_config_t;

/**
 * @brief Create one preconfigured remote app facade over a remote session.
 */
CXX_C_API turbo_agent_remote_app_t *
turbo_agent_remote_app_create(const turbo_agent_remote_app_config_t *config);

/**
 * @brief Destroy one remote app facade.
 */
CXX_C_API void turbo_agent_remote_app_destroy(turbo_agent_remote_app_t *app);

/**
 * @brief Return the owned remote session facade.
 */
CXX_C_API turbo_agent_remote_session_t *
turbo_agent_remote_app_session(const turbo_agent_remote_app_t *app);

/**
 * @brief Return the configured remote graph name.
 */
...
/**
 * @brief Return the last remote run id observed by the app.
 */
CXX_C_API const char *turbo_agent_remote_app_last_run_id(const turbo_agent_remote_app_t *app);

/**
 * @brief Return the most recent non-null checkpoint id observed by the app.
 */
CXX_C_API const char *turbo_agent_remote_app_last_checkpoint_id(
    const turbo_agent_remote_app_t *app);

/**
 * @brief Load remote startup diagnostics for the configured graph.
 *
...
/**
 * @brief Start the configured remote graph from canonical prompt messages and return the final answer text.
 *
 * Returned text is allocated and owned by caller.
 */
...
/**
 * @brief Start the configured remote graph from user text and parse the final JSON output.
 */
...
/**
 * @brief Load one canonical memory record through the remote app facade.
 */
...
/**
 * @brief Persist one canonical memory record through the remote app facade.
 */
...
/**
 * @brief Query long-term memory records through the remote app facade with explicit options.
 */
...
/**
 * @brief Load the newest run record for the app thread through the remote
...
/**
 * @brief Load the newest interrupted run record for the app thread through the
 * remote observability bundle.
 */
...
/**
 * @brief List child runs for one parent run through the remote runtime.
 */
