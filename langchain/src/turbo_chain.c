...
typedef struct {
  ...
  turbo_chain_json_value_step_fn json_value_fn;
  void *user_data;
...
static int turbo_chain_emit_json_value_event(turbo_chain_exec_ctx_t *ctx,
                                       json_value_t *event) {
  if (!event) {
...
static json_value_t *turbo_chain_build_model_tool_calls_json_value(
  ...
  if (tool_name && tool_name[0] != '\0') {
    ...
    if (!tool_call || !call_id_value || !name_value || !arguments_value ||
        turbo_runtime_json_object_set(tool_call, "call_id", call_id_value) !=
            TURBO_RUNTIME_JSON_OK ||
        turbo_runtime_json_object_set(tool_call, "name", name_value) !=
            TURBO_RUNTIME_JSON_OK ||
        turbo_runtime_json_object_set(tool_call, "arguments", arguments_value) !=
            TURBO_RUNTIME_JSON_OK ||
        turbo_runtime_json_array_append(tool_calls, tool_call) != TURBO_RUNTIME_JSON_OK) {
      turbo_runtime_json_destroy(tool_call);
      turbo_runtime_json_destroy(tool_calls);
      return NULL;
    }
...
static int turbo_chain_emit_model_event_json_value(turbo_chain_exec_ctx_t *ctx, const char *output_text,
                                             const char *tool_name,
                                             const json_value_t *tool_arguments,
  ...
  tool_calls = turbo_chain_build_model_tool_calls_json_value(ctx, tool_name, tool_arguments,
                                                       tool_arguments_json);
  if (!tool_calls) {
    return TURBO_CHAIN_OUT_OF_MEMORY;
  }

  event = turbo_event_model_create_json_value("", output_text ? output_text : "", tool_calls);
  turbo_runtime_json_destroy(tool_calls);
  return turbo_chain_emit_json_value_event(ctx, event);
}

static int turbo_chain_emit_tool_result_event_json_value(
  ...
  event = turbo_event_tool_result_create_json_value(name, arguments_json, output, output_value, status);
  return turbo_chain_emit_json_value_event(ctx, event);
}
...
static turbo_chain_status_t turbo_chain_json_value_state_to_json(
    const json_value_t *state, json_value_t **out_json) {
  if (!out_json) {
...
static json_value_t *
turbo_chain_json_value_state_require_object(json_value_t *state, const char *key) {
  json_value_t *value;
  ...
  if (turbo_runtime_json_object_set(state, key, value) != TURBO_RUNTIME_JSON_OK) {
...
static json_value_t *
turbo_chain_json_value_state_require_array(json_value_t *state, const char *key) {
  json_value_t *value;
...
static json_value_t *
turbo_chain_json_value_state_input(json_value_t *state) {
  return turbo_chain_json_value_state_require_object(state, "input");
}

static json_value_t *
turbo_chain_json_value_state_messages(json_value_t *state) {
  return turbo_chain_json_value_state_require_array(state, "messages");
}

static json_value_t *
turbo_chain_json_value_state_model_outputs(json_value_t *state) {
  return turbo_chain_json_value_state_require_array(state, "model_outputs");
}

static json_value_t *
turbo_chain_json_value_state_tool_requests(json_value_t *state) {
  return turbo_chain_json_value_state_require_array(state, "tool_requests");
}

static json_value_t *
turbo_chain_json_value_state_tool_results(json_value_t *state) {
  return turbo_chain_json_value_state_require_array(state, "tool_results");
}

static json_value_t *turbo_chain_json_value_parse_json_value(const char *json_text) {
  json_value_t *json_value = NULL;
  json_value_t *cloned_value;
...
static turbo_chain_status_t turbo_chain_append_message_json_value(
    json_value_t *state, const char *role, const char *content) {
  json_value_t *messages;

  messages = turbo_chain_json_value_state_messages(state);
  if (!messages) {
    return TURBO_CHAIN_OUT_OF_MEMORY;
  }

  return turbo_prompt_messages_append_json_value(messages, role, content) == TURBO_PROMPT_OK
             ? TURBO_CHAIN_OK
             : TURBO_CHAIN_OUT_OF_MEMORY;
}
...
static turbo_chain_status_t
turbo_chain_store_tool_request_json_value(json_value_t *state, const char *tool_name,
...
static turbo_chain_status_t
turbo_chain_store_tool_request_json_value_state(json_value_t *state,
                                          const char *tool_name,
                                          const json_value_t *arguments) {
  json_value_t *tool_requests;
  json_value_t *tool_request;
  json_value_t *name_value;
  json_value_t *arguments_copy = NULL;
  json_value_t *arguments_json_value;
  char *arguments_json = NULL;

  tool_requests = turbo_chain_json_value_state_tool_requests(state);
  if (!tool_requests) {
    return TURBO_CHAIN_OUT_OF_MEMORY;
  }
  ...
  if (turbo_runtime_json_object_set(tool_request, "name", name_value) !=
      TURBO_RUNTIME_JSON_OK) {
    turbo_runtime_json_destroy(tool_request);
    return TURBO_CHAIN_OUT_OF_MEMORY;
  }
  ...
  if (arguments) {
    ...
    if (turbo_runtime_json_object_set(tool_request, "arguments_json", arguments_json_value) !=
        TURBO_RUNTIME_JSON_OK) {
      turbo_runtime_json_destroy(arguments_json_value);
      turbo_runtime_json_destroy(tool_request);
      return TURBO_CHAIN_OUT_OF_MEMORY;
    }
  } else {
  ...
  if (turbo_runtime_json_array_append(tool_requests, tool_request) !=
      TURBO_RUNTIME_JSON_OK) {
    turbo_runtime_json_destroy(tool_request);
    return TURBO_CHAIN_OUT_OF_MEMORY;
  }
...
static turbo_chain_status_t
turbo_chain_store_tool_result_json_value(json_value_t *state, const char *name,
                                   const json_value_t *arguments,
...
static turbo_chain_status_t
turbo_chain_store_tool_result_json_value_state(json_value_t *state, const char *name,
                                         const json_value_t *arguments,
                                         const json_value_t *output_value,
                                         const char *output_text) {
  json_value_t *tool_results;
  json_value_t *result_entry;
  json_value_t *name_value;
  json_value_t *output_string;
  json_value_t *arguments_copy = NULL;
  json_value_t *output_copy = NULL;
  json_value_t *arguments_json_value;
  char *arguments_json = NULL;

  tool_results = turbo_chain_json_value_state_tool_results(state);
  if (!tool_results) {
    return TURBO_CHAIN_OUT_OF_MEMORY;
  }
  ...
  if (!result_entry || !name_value || !output_string) {
    ...
    turbo_runtime_json_destroy(name_value);
    turbo_runtime_json_destroy(output_string);
    return TURBO_CHAIN_OUT_OF_MEMORY;
  }

  if (turbo_runtime_json_object_set(result_entry, "name", name_value) !=
      TURBO_RUNTIME_JSON_OK) {
    turbo_runtime_json_destroy(result_entry);
    return TURBO_CHAIN_OUT_OF_MEMORY;
  }
  if (turbo_runtime_json_object_set(result_entry, "output", output_string) !=
      TURBO_RUNTIME_JSON_OK) {
    turbo_runtime_json_destroy(result_entry);
    return TURBO_CHAIN_OUT_OF_MEMORY;
  }
  ...
  if (arguments) {
    ...
    if (turbo_runtime_json_object_set(result_entry, "arguments_json", arguments_json_value) !=
        TURBO_RUNTIME_JSON_OK) {
      turbo_runtime_json_destroy(arguments_json_value);
      turbo_runtime_json_destroy(result_entry);
      return TURBO_CHAIN_OUT_OF_MEMORY;
  ...
  if (output_value) {
    ...
    if (turbo_runtime_json_object_set(result_entry, "output_value", output_copy) !=
        TURBO_RUNTIME_JSON_OK) {
      turbo_runtime_json_destroy(output_copy);
      turbo_runtime_json_destroy(result_entry);
      return TURBO_CHAIN_OUT_OF_MEMORY;
    }
  }

  if (turbo_runtime_json_array_append(tool_results, result_entry) !=
      TURBO_RUNTIME_JSON_OK) {
    turbo_runtime_json_destroy(result_entry);
    return TURBO_CHAIN_OUT_OF_MEMORY;
  }
...
static int turbo_chain_prompt_step_run(turbo_chain_exec_ctx_t *ctx, void *user_data) {
  ...
  if (!ctx || (!ctx->state && !ctx->json_value_state) || !prompt) {
    return TURBO_CHAIN_INVALID_ARGUMENT;
  }

  if (ctx->json_value_state) {
    rendered = turbo_prompt_render_template_json_value(prompt->template_text,
                                                 turbo_chain_json_value_state_input(ctx->json_value_state));
  } else {
  ...
  status = ctx->json_value_state ? turbo_chain_append_message_json_value(ctx->json_value_state, prompt->role, rendered)
...
static int turbo_chain_model_step_run(turbo_chain_exec_ctx_t *ctx, void *user_data) {
  ...
  turbo_model_json_value_result_t json_value_result = {0};
  json_value_t *messages_json_value = NULL;
  json_value_t *arguments_json_value = NULL;
  const json_value_t *event_tool_arguments = NULL;
  json_value_t *model_outputs;
  json_value_t *output_entry;
  int status;

  if (!ctx || (!ctx->state && !ctx->json_value_state) || !step ||
      (!step->model.invoke && !step->model.invoke_json_value)) {
    return TURBO_CHAIN_INVALID_ARGUMENT;
  }

  if (ctx->json_value_state) {
    messages_json_value = turbo_chain_json_value_state_messages(ctx->json_value_state);
  ...
  if (step->model.invoke_json_value) {
    status = step->model.invoke_json_value(step->model.user_data, messages_json_value, step->tools,
                                     &json_value_result);
    if (!ctx->json_value_state) {
  ...
  if ((step->model.invoke_json_value ? json_value_result.output_text : result.output_text)) {
    const char *output_text =
        step->model.invoke_json_value ? json_value_result.output_text : result.output_text;
    ...
    if (ctx->json_value_state) {
      ...
      if (turbo_chain_append_message_json_value(ctx->json_value_state, "assistant", output_text) != TURBO_CHAIN_OK ||
        ...
        turbo_runtime_json_destroy(output_entry_json_value);
        turbo_runtime_json_destroy(text_value);
        return TURBO_CHAIN_OUT_OF_MEMORY;
      }

      if (turbo_runtime_json_object_set(output_entry_json_value, "text", text_value) !=
          TURBO_RUNTIME_JSON_OK) {
        turbo_runtime_json_destroy(output_entry_json_value);
        return TURBO_CHAIN_OUT_OF_MEMORY;
      }
      if (turbo_runtime_json_array_append(model_outputs_json_value, output_entry_json_value) !=
          TURBO_RUNTIME_JSON_OK) {
        turbo_runtime_json_destroy(output_entry_json_value);
        return TURBO_CHAIN_OUT_OF_MEMORY;
      }
  ...
  if (step->model.invoke_json_value ? json_value_result.tool_name != NULL : result.tool_name != NULL) {
    const char *tool_name = step->model.invoke_json_value ? json_value_result.tool_name : result.tool_name;
    turbo_chain_status_t chain_status;

    if (step->model.invoke_json_value) {
      event_tool_arguments = json_value_result.tool_arguments;
      chain_status =
          ctx->json_value_state ? turbo_chain_store_tool_request_json_value_state(ctx->json_value_state, tool_name,
                                                                      json_value_result.tool_arguments)
                          : turbo_chain_store_tool_request_json_value(ctx->state, tool_name,
                                                                json_value_result.tool_arguments);
    } else {
      if (ctx->json_value_state) {
        if (result.tool_arguments_json) {
          arguments_json_value = turbo_chain_json_value_parse_json_value(result.tool_arguments_json);
          if (!arguments_json_value) {
            return TURBO_CHAIN_ERROR;
          }
        }
        event_tool_arguments = arguments_json_value;
        chain_status =
            turbo_chain_store_tool_request_json_value_state(ctx->json_value_state, tool_name, arguments_json_value);
      } else {
        chain_status =
    ...
    if (chain_status != TURBO_CHAIN_OK) {
      turbo_runtime_json_destroy(arguments_json_value);
      arguments_json_value = NULL;
      turbo_runtime_json_destroy(json_value_result.tool_arguments);
      json_value_result.tool_arguments = NULL;
      return chain_status;
    }
  }

  if ((step->model.invoke_json_value ? json_value_result.output_text != NULL : result.output_text != NULL) ||
      (step->model.invoke_json_value ? json_value_result.tool_name != NULL : result.tool_name != NULL)) {
    status = step->model.invoke_json_value
                 ? turbo_chain_emit_model_event_json_value(ctx, json_value_result.output_text,
                                                     json_value_result.tool_name, event_tool_arguments,
                                                     NULL)
                 : turbo_chain_emit_model_event_json_value(ctx, result.output_text, result.tool_name,
...
static int turbo_chain_tool_step_run(turbo_chain_exec_ctx_t *ctx, void *user_data) {
  ...
  if (!ctx || (!ctx->state && !ctx->json_value_state) || !step || !step->tools) {
    return TURBO_CHAIN_INVALID_ARGUMENT;
  }

  if (ctx->json_value_state) {
    json_value_t *tool_requests_json_value =
        turbo_chain_json_value_state_tool_requests(ctx->json_value_state);
    json_value_t *tool_results_json_value =
        turbo_chain_json_value_state_tool_results(ctx->json_value_state);
    const json_value_t *tool_request_json_value;
    const json_value_t *tool_arguments_value;
    json_value_t *output_json = NULL;

    if (!tool_requests_json_value || !tool_results_json_value) {
      return TURBO_CHAIN_OUT_OF_MEMORY;
    }

    request_index = turbo_runtime_json_value_size(tool_results_json_value);
    if (request_index >= turbo_runtime_json_value_size(tool_requests_json_value)) {
      return TURBO_CHAIN_OK;
    }
    ...
    if (tool_arguments_value) {
      ...
      if (tool_status == TURBO_TOOL_OK) {
        ...
        chain_status = turbo_chain_append_message_json_value(ctx->json_value_state, "tool", output);
        if (chain_status != TURBO_CHAIN_OK) {
          turbo_runtime_json_destroy(output_json_value);
          free(output);
          return chain_status;
        ...
        chain_status = turbo_chain_store_tool_result_json_value_state(
            ctx->json_value_state, name, tool_arguments_value, output_json_value, output);
        if (chain_status == TURBO_CHAIN_OK) {
          chain_status = turbo_chain_emit_tool_result_event_json_value(
              ctx, name, arguments_json ? arguments_json : "{}", output, output_json_value, tool_status);
        }
        turbo_runtime_json_destroy(output_json_value);
        free(output);
        return chain_status;
    ...
    chain_status = turbo_chain_append_message_json_value(ctx->json_value_state, "tool", output ? output : "");
    if (chain_status != TURBO_CHAIN_OK) {
      free(output);
      return chain_status;
    }
    ...
    chain_status = turbo_chain_store_tool_result_json_value_state(
        ctx->json_value_state, name, tool_arguments_value, NULL, output ? output : "");
    if (chain_status == TURBO_CHAIN_OK) {
      chain_status = turbo_chain_emit_tool_result_event_json_value(
  ...
  if (arguments_value) {
    ...
    tool_status = turbo_tool_registry_execute_json_value(step->tools, name, arguments_json_value, &output_json_value);
    if (tool_status != TURBO_TOOL_OK) {
      turbo_runtime_json_destroy(arguments_json_value);
      return TURBO_CHAIN_ERROR;
    }
    ...
    chain_status =
        turbo_chain_store_tool_result_json_value(ctx->state, name, arguments_json_value, output_json_value, output);
    if (chain_status == TURBO_CHAIN_OK) {
      chain_status = turbo_chain_emit_tool_result_event_json_value(
          ctx, name, arguments_json ? arguments_json : "{}", output, output_json_value, tool_status);
    }
...
turbo_chain_status_t turbo_chain_add_prompt_step(turbo_chain_t *chain, const char *name,
  ...
  chain->steps[chain->step_count - 1].json_value_fn = turbo_chain_prompt_step_run;
  return TURBO_CHAIN_OK;
}
...
turbo_chain_status_t turbo_chain_add_model_step(turbo_chain_t *chain, const char *name,
  ...
  if (!chain || !name || !model || (!model->invoke && !model->invoke_json_value)) {
    return TURBO_CHAIN_INVALID_ARGUMENT;
  }
  ...
  chain->steps[chain->step_count - 1].json_value_fn = turbo_chain_model_step_run;
  return TURBO_CHAIN_OK;
}
...
turbo_chain_status_t turbo_chain_add_tool_step(turbo_chain_t *chain, const char *name,
  ...
  chain->steps[chain->step_count - 1].json_value_fn = turbo_chain_tool_step_run;
  return TURBO_CHAIN_OK;
}
...
turbo_chain_run_json_value(turbo_chain_t *chain, const json_value_t *state,
  ...
  return turbo_chain_run_json_value_stream(chain, state, NULL, NULL, out_state);
}

turbo_chain_status_t turbo_chain_run_json_value_stream(
  ...
  *out_state = NULL;
  ...
  for (i = 0; i < chain->step_count; ++i) {
    memset(&ctx, 0, sizeof(ctx));
    ctx.chain = chain;
    ctx.json_value_state = bound_state;
    ctx.step = i;
    ctx.step_name = chain->steps[i].name;
    ctx.event_sink = event_sink;
    ctx.event_sink_user_data = event_sink_user_data;

    if (!chain->steps[i].json_value_fn) {
      turbo_runtime_json_destroy(bound_state);
      return TURBO_CHAIN_INVALID_ARGUMENT;
    }

    status = chain->steps[i].json_value_fn(&ctx, chain->steps[i].user_data);
    if (status == TURBO_CHAIN_STOP || ctx.stop) {
...
turbo_chain_run_json_value_log(turbo_chain_t *chain, const json_value_t *state,
  ...
  status =
      turbo_chain_run_json_value_stream(chain, state, turbo_event_log_capture_json_value, log, out_state);
...
json_value_t *turbo_chain_state_create_json_value(void) {
  ...
  if (!state || !input || !messages || !tool_results || !model_outputs || !tool_requests) {
    ...
    turbo_runtime_json_destroy(tool_results);
    turbo_runtime_json_destroy(model_outputs);
    turbo_runtime_json_destroy(tool_requests);
    turbo_runtime_json_destroy(state);
    return NULL;
  }

  if (turbo_runtime_json_object_set(state, "input", input) != TURBO_RUNTIME_JSON_OK ||
      turbo_runtime_json_object_set(state, "messages", messages) != TURBO_RUNTIME_JSON_OK ||
      turbo_runtime_json_object_set(state, "tool_results", tool_results) != TURBO_RUNTIME_JSON_OK ||
      turbo_runtime_json_object_set(state, "model_outputs", model_outputs) != TURBO_RUNTIME_JSON_OK ||
      turbo_runtime_json_object_set(state, "tool_requests", tool_requests) !=
          TURBO_RUNTIME_JSON_OK) {
    turbo_runtime_json_destroy(state);
    return NULL;
  }
