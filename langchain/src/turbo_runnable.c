...
static const char *turbo_runnable_agent_text_input(
  ...
  text = turbo_runtime_json_value_as_string(input);
  if (text) {
    return text;
  }
...
static const json_value_t *turbo_runnable_agent_messages_input(
    const json_value_t *input) {
...
static int turbo_runnable_agent_result_json_value(
    json_value_t **summary_json, json_value_t **state,
    char *output_text, json_value_t **out_output) {
  json_value_t *result;
  ...
  if (turbo_runtime_json_object_set(result, "state", *state) !=
      TURBO_RUNTIME_JSON_OK) {
    turbo_runtime_json_destroy(result);
    return -1;
  }
  *state = NULL;
  ...
  if (!text ||
      turbo_runtime_json_object_set(result, "output_text", text) !=
          TURBO_RUNTIME_JSON_OK) {
    turbo_runtime_json_destroy(text);
    turbo_runtime_json_destroy(result);
    return -1;
  }
...
static int turbo_runnable_pipe_invoke_json_value(const json_value_t *input,
  ...
  rc = turbo_runnable_invoke_json_value(pipe->first, input, &middle);
  if (rc != 0) {
    return rc;
  }

  rc = turbo_runnable_invoke_json_value(pipe->second, middle, out_output);
  turbo_runtime_json_destroy(middle);
  return rc;
}

static int turbo_runnable_pipe_invoke_json_value_stream(
  ...
  rc = turbo_runnable_invoke_json_value_stream(pipe->first, input, event_sink, event_sink_user_data,
                                         &middle);
  if (rc != 0) {
    return rc;
  }

  rc = turbo_runnable_invoke_json_value_stream(pipe->second, middle, event_sink, event_sink_user_data,
                                         out_output);
  turbo_runtime_json_destroy(middle);
  return rc;
}

static int turbo_runnable_pipe_batch_json_value(
  ...
  rc = turbo_runnable_batch_json_value(pipe->first, inputs, &middle);
  if (rc != 0) {
    return rc;
  }

  rc = turbo_runnable_batch_json_value(pipe->second, middle, out_outputs);
  turbo_runtime_json_destroy(middle);
  return rc;
}
...
static int turbo_runnable_wrap_invoke_common(
    ...
    int use_stream,
    json_value_t **out_output) {
  const json_value_t *effective_input = input;
  json_value_t *replacement_input = NULL;
  json_value_t *inner_output = NULL;
  json_value_t *replacement_output = NULL;
  int rc;
  ...
  *out_output = NULL;
  if (adapter->config.before_invoke) {
    ...
    if (rc != 0) {
      if (replacement_input && replacement_input != input) {
        turbo_runtime_json_destroy(replacement_input);
      }
      return rc;
    }
  ...
  if (use_stream) {
    rc = turbo_runnable_invoke_json_value_stream(adapter->inner, effective_input, event_sink,
                                           event_sink_user_data, &inner_output);
  } else {
    rc = turbo_runnable_invoke_json_value(adapter->inner, effective_input, &inner_output);
  }
  if (rc != 0 || !inner_output) {
    if (replacement_input && replacement_input != input) {
      turbo_runtime_json_destroy(replacement_input);
    }
    turbo_runtime_json_destroy(inner_output);
    return rc != 0 ? rc : -1;
  }
  ...
  if (adapter->config.after_invoke) {
    ...
    if (rc != 0) {
      if (replacement_output && replacement_output != inner_output) {
        turbo_runtime_json_destroy(replacement_output);
      }
      turbo_runtime_json_destroy(inner_output);
      if (replacement_input && replacement_input != input) {
        turbo_runtime_json_destroy(replacement_input);
...
static int turbo_runnable_wrap_invoke_json_value(
    const json_value_t *input,
...
static int turbo_runnable_wrap_invoke_json_value_stream(
    const json_value_t *input, turbo_event_sink_json_value_fn event_sink,
    void *event_sink_user_data, json_value_t **out_output,
    void *user_data) {
  return turbo_runnable_wrap_invoke_common((turbo_runnable_wrap_adapter_t *)user_data,
...
static int turbo_runnable_wrap_batch_json_value(
    const json_value_t *inputs,
    json_value_t **out_outputs, void *user_data) {
  turbo_runnable_wrap_adapter_t *adapter = (turbo_runnable_wrap_adapter_t *)user_data;
  ...
  *out_outputs = NULL;
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (turbo_runtime_json_array_append(outputs, output) !=
        TURBO_RUNTIME_JSON_OK) {
      turbo_runtime_json_destroy(output);
      turbo_runtime_json_destroy(outputs);
      return -1;
...
static int turbo_runnable_chain_invoke_json_value(const json_value_t *input,
                                            json_value_t **out_output,
                                            void *user_data) {
  ...
  return turbo_chain_run_json_value(adapter->chain, input, out_output) == TURBO_CHAIN_OK ? 0 : -1;
}

static int turbo_runnable_chain_invoke_json_value_stream(
    const json_value_t *input, turbo_event_sink_json_value_fn event_sink,
    void *event_sink_user_data, json_value_t **out_output, void *user_data) {
  json_value_t *event;
  int rc;
  ...
  if (event_sink) {
    event = turbo_event_trace_create_json_value("runnable.chain", "start", "", 0);
    if (event) {
  ...
  {
    turbo_runnable_chain_adapter_t *adapter = (turbo_runnable_chain_adapter_t *)user_data;
    rc = turbo_chain_run_json_value_stream(adapter->chain, input, event_sink, event_sink_user_data,
  ...
  if (event_sink) {
    event = turbo_event_trace_create_json_value("runnable.chain", "finish", "", rc);
    if (event) {
      event_sink(event, event_sink_user_data);
      turbo_runtime_json_destroy(event);
    }
...
static int turbo_runnable_graph_invoke_json_value(const json_value_t *input,
                                            json_value_t **out_output,
                                            void *user_data) {
  ...
  status = turbo_graph_run_json_value(adapter->graph, input, &adapter->options, &result, out_output);
...
static int turbo_runnable_graph_invoke_json_value_stream(
    const json_value_t *input, turbo_event_sink_json_value_fn event_sink,
    void *event_sink_user_data, json_value_t **out_output, void *user_data) {
  json_value_t *event;
  int rc;
  ...
  if (event_sink) {
    event = turbo_event_trace_create_json_value("runnable.graph", "start", "", 0);
    if (event) {
      event_sink(event, event_sink_user_data);
      turbo_runtime_json_destroy(event);
    }
...

  {
    ...
    turbo_graph_run_result_t result = {0};
    turbo_graph_exec_status_t status =
        turbo_graph_run_json_value_stream(adapter->graph, input, &adapter->options, event_sink,
  ...
  if (event_sink) {
    event = turbo_event_trace_create_json_value("runnable.graph", "finish", "", rc);
    if (event) {
      event_sink(event, event_sink_user_data);
      turbo_runtime_json_destroy(event);
    }
...
static int turbo_runnable_state_graph_invoke_json_value(
    const json_value_t *input,
    json_value_t **out_output, void *user_data) {
  turbo_runnable_state_graph_adapter_t *adapter =
      (turbo_runnable_state_graph_adapter_t *)user_data;
...
static int turbo_runnable_state_graph_invoke_json_value_stream(
    const json_value_t *input, turbo_event_sink_json_value_fn event_sink,
    void *event_sink_user_data, json_value_t **out_output, void *user_data) {
  json_value_t *event;
  int rc;
  ...
  if (event_sink) {
    event = turbo_event_trace_create_json_value("runnable.state_graph", "start", "", 0);
    if (event) {
      event_sink(event, event_sink_user_data);
      turbo_runtime_json_destroy(event);
    }
  ...
  rc = turbo_runnable_state_graph_invoke_json_value(input, out_output, user_data);

  if (event_sink) {
    event = turbo_event_trace_create_json_value("runnable.state_graph", "finish", "", rc);
    if (event) {
      event_sink(event, event_sink_user_data);
      turbo_runtime_json_destroy(event);
    }
...
static int turbo_runnable_agent_session_invoke_json_value(
    const json_value_t *input,
    json_value_t **out_output, void *user_data) {
...
static int turbo_runnable_agent_session_invoke_json_value_stream(
    const json_value_t *input, turbo_event_sink_json_value_fn event_sink,
    void *event_sink_user_data, json_value_t **out_output,
    void *user_data) {
  turbo_runnable_agent_session_adapter_t *adapter =
...
static int turbo_runnable_agent_app_invoke_json_value(
    const json_value_t *input,
    json_value_t **out_output, void *user_data) {
...
static int turbo_runnable_agent_app_invoke_json_value_stream(
    const json_value_t *input, turbo_event_sink_json_value_fn event_sink,
    void *event_sink_user_data, json_value_t **out_output,
    void *user_data) {
  turbo_runnable_agent_app_adapter_t *adapter =
...
turbo_runnable_t *turbo_runnable_create(const turbo_runnable_config_t *config) {
  ...
  if (!config || !config->invoke_json_value) {
    return NULL;
  }
  ...
  runnable->invoke_json_value = config->invoke_json_value;
  runnable->invoke_json_value_stream = config->invoke_json_value_stream;
  runnable->batch_json_value = config->batch_json_value;
...
int turbo_runnable_invoke_json_value(const turbo_runnable_t *runnable,
                               const json_value_t *input,
                               json_value_t **out_output) {
  if (!runnable || !runnable->invoke_json_value || !out_output) {
    return -1;
  }

  *out_output = NULL;
  return runnable->invoke_json_value(input, out_output, runnable->user_data);
}

int turbo_runnable_invoke_json_value_stream(
  ...
  if (runnable->invoke_json_value_stream) {
    return runnable->invoke_json_value_stream(input, event_sink, event_sink_user_data, out_output,
                                        runnable->user_data);
  }

  if (event_sink) {
    event = turbo_event_trace_create_json_value("runnable.invoke", "start", "", 0);
    if (event) {
      event_sink(event, event_sink_user_data);
      turbo_runtime_json_destroy(event);
    }
  ...
  rc = turbo_runnable_invoke_json_value(runnable, input, out_output);

  if (event_sink) {
    event = turbo_event_trace_create_json_value("runnable.invoke", "finish", "", rc);
    if (event) {
      event_sink(event, event_sink_user_data);
      turbo_runtime_json_destroy(event);
    }
...
int turbo_runnable_invoke_json_value_log(const turbo_runnable_t *runnable,
                                   const json_value_t *input,
  ...
  rc = turbo_runnable_invoke_json_value_stream(runnable, input, turbo_event_log_capture_json_value, log,
                                         out_output);
  if (turbo_event_log_status(log) != TURBO_EVENT_LOG_OK) {
    if (out_output && *out_output) {
      turbo_runtime_json_destroy(*out_output);
      *out_output = NULL;
    }
    return -1;
  }
...
int turbo_runnable_batch_json_value(const turbo_runnable_t *runnable,
                              const json_value_t *inputs,
...
turbo_runnable_t *turbo_runnable_pipe(const turbo_runnable_t *first,
  ...
  config.invoke_json_value = turbo_runnable_pipe_invoke_json_value;
  config.invoke_json_value_stream = turbo_runnable_pipe_invoke_json_value_stream;
  config.batch_json_value = turbo_runnable_pipe_batch_json_value;
  config.user_data = pipe;
  config.user_data_free = free;
  return turbo_runnable_create(&config);
}

turbo_runnable_t *turbo_runnable_wrap_json_value(
  ...
  memset(&config, 0, sizeof(config));
  config.invoke_json_value = turbo_runnable_wrap_invoke_json_value;
  config.invoke_json_value_stream = turbo_runnable_wrap_invoke_json_value_stream;
  config.batch_json_value = turbo_runnable_wrap_batch_json_value;
...
turbo_runnable_t *turbo_runnable_from_chain(turbo_chain_t *chain) {
  ...
  adapter->chain = chain;
  memset(&config, 0, sizeof(config));
  config.invoke_json_value = turbo_runnable_chain_invoke_json_value;
  config.invoke_json_value_stream = turbo_runnable_chain_invoke_json_value_stream;
  config.user_data = adapter;
  config.user_data_free = free;
  return turbo_runnable_create(&config);
}
...
turbo_runnable_t *turbo_runnable_from_agent_session(
  ...
  memset(&config, 0, sizeof(config));
  config.invoke_json_value = turbo_runnable_agent_session_invoke_json_value;
  config.invoke_json_value_stream = turbo_runnable_agent_session_invoke_json_value_stream;
  config.user_data = adapter;
  config.user_data_free = free;
  {
...
turbo_runnable_t *turbo_runnable_from_agent_app(
  ...
  memset(&config, 0, sizeof(config));
  config.invoke_json_value = turbo_runnable_agent_app_invoke_json_value;
  config.invoke_json_value_stream = turbo_runnable_agent_app_invoke_json_value_stream;
  config.user_data = adapter;
  config.user_data_free = free;
  {
    turbo_runnable_t *runnable = turbo_runnable_create(&config);
...
turbo_runnable_t *turbo_runnable_from_state_graph(
  ...
  memset(&config, 0, sizeof(config));
  config.invoke_json_value = turbo_runnable_state_graph_invoke_json_value;
  config.invoke_json_value_stream = turbo_runnable_state_graph_invoke_json_value_stream;
  config.user_data = adapter;
...
turbo_runnable_t *turbo_runnable_from_graph(turbo_graph_t *graph,
  ...
  memset(&config, 0, sizeof(config));
  config.invoke_json_value = turbo_runnable_graph_invoke_json_value;
  config.invoke_json_value_stream = turbo_runnable_graph_invoke_json_value_stream;
  config.user_data = adapter;
  config.user_data_free = free;
  return turbo_runnable_create(&config);
}
