...
static int wrap_before_bind(const turbo_runtime_data_bind_value_t *input,
                            turbo_runtime_data_bind_value_t **out_input,
                            void *user_data) {
  runnable_wrap_counter_t *counter = (runnable_wrap_counter_t *)user_data;
  turbo_runtime_data_bind_value_t *replacement;
  ...
  replacement = turbo_runtime_data_bind_value_clone(input);
  check_not_null(replacement);
  check_int_eq(turbo_runtime_data_bind_object_set(
                   replacement, "value",
                   turbo_runtime_data_bind_value_create_string("wrapped")),
               TURBO_RUNTIME_DATA_BIND_OK);
  *out_input = replacement;
  return 0;
}

static int wrap_after_bind(const turbo_runtime_data_bind_value_t *input,
  ...
  check_str_eq(turbo_runtime_data_bind_value_as_string(
                   turbo_runtime_data_bind_object_get(input, "value")),
               "wrapped");

  counter->after_count++;
  replacement = turbo_runtime_data_bind_value_clone(output);
  check_not_null(replacement);
  check_int_eq(turbo_runtime_data_bind_object_set(
                   replacement, "after", turbo_runtime_data_bind_value_create_bool(1)),
               TURBO_RUNTIME_DATA_BIND_OK);
...
static int state_graph_mark_bind(turbo_state_graph_exec_ctx_t *ctx, void *user_data) {
  (void)user_data;
  return turbo_runtime_data_bind_object_set(
             ctx->update, "state_done", turbo_runtime_data_bind_value_create_bool(1)) ==
                 TURBO_RUNTIME_DATA_BIND_OK
             ? 0
             : -1;
}
...
spec("turbo runnable runtime") {
  describe("bind-native composition") {
    ...
    it("should batch invoke bind-native runnables") {
      ...
      turbo_runtime_data_bind_value_t *inputs = turbo_runtime_data_bind_value_create_array();
      turbo_runtime_data_bind_value_t *first_input =
          turbo_runtime_data_bind_value_create_object();
      turbo_runtime_data_bind_value_t *second_input =
          turbo_runtime_data_bind_value_create_object();
      turbo_runtime_data_bind_value_t *outputs = NULL;
      const turbo_runtime_data_bind_value_t *first_output;
      const turbo_runtime_data_bind_value_t *second_output;
      ...
      check_int_eq(turbo_runtime_data_bind_object_set(
                       first_input, "value",
                       turbo_runtime_data_bind_value_create_string("first")),
                   TURBO_RUNTIME_DATA_BIND_OK);
      check_int_eq(turbo_runtime_data_bind_object_set(
                       second_input, "value",
                       turbo_runtime_data_bind_value_create_string("second")),
                   TURBO_RUNTIME_DATA_BIND_OK);
      check_int_eq(turbo_runtime_data_bind_array_append(inputs, first_input),
                   TURBO_RUNTIME_DATA_BIND_OK);
      check_int_eq(turbo_runtime_data_bind_array_append(inputs, second_input),
                   TURBO_RUNTIME_DATA_BIND_OK);

      check_int_eq(turbo_runnable_batch_bind(runnable, inputs, &outputs), 0);
      check_size_eq(turbo_runtime_data_bind_value_size(outputs), 2);
      first_output = turbo_runtime_data_bind_array_get(outputs, 0);
      second_output = turbo_runtime_data_bind_array_get(outputs, 1);
      check_int_eq((int)turbo_runtime_data_bind_value_as_int64(
                       turbo_runtime_data_bind_object_get(first_output, "count"), 0),
                   1);
      check_int_eq((int)turbo_runtime_data_bind_value_as_int64(
                       turbo_runtime_data_bind_object_get(second_output, "count"), 0),
                   2);
      check_int_eq(counter.count, 2);

      turbo_runtime_data_bind_value_destroy(outputs);
      turbo_runtime_data_bind_value_destroy(inputs);
    ...
    it("should batch through runnable pipes") {
      ...
      check_int_eq(turbo_runnable_batch_bind(pipe, inputs, &outputs), 0);
      check_size_eq(turbo_runtime_data_bind_value_size(outputs), 2);
      check_int_eq(first_counter.count, 2);
      check_int_eq(second_counter.count, 2);
      check_int_eq((int)turbo_runtime_data_bind_value_as_int64(
                       turbo_runtime_data_bind_object_get(
                           turbo_runtime_data_bind_array_get(outputs, 0), "count"),
                       0),
                   1);
      check_int_eq((int)turbo_runtime_data_bind_value_as_int64(
                       turbo_runtime_data_bind_object_get(
                           turbo_runtime_data_bind_array_get(outputs, 1), "count"),
                       0),
                   2);
    ...
    it("should wrap runnable invoke input and output") {
      ...
      turbo_runnable_wrap_config_t wrap_config = {
          .before_invoke = wrap_before_bind,
          .after_invoke = wrap_after_bind,
          .user_data = &wrap_counter,
          .user_data_free = NULL};
      turbo_runnable_t *inner = turbo_runnable_create(&inner_config);
      turbo_runnable_t *wrapped = turbo_runnable_wrap_bind(inner, &wrap_config);
      turbo_runtime_data_bind_value_t *input = turbo_runtime_data_bind_value_create_object();
      turbo_runtime_data_bind_value_t *output = NULL;
      ...
      check_int_eq(turbo_runtime_data_bind_object_set(
                       input, "value",
                       turbo_runtime_data_bind_value_create_string("original")),
                   TURBO_RUNTIME_DATA_BIND_OK);

      check_int_eq(turbo_runnable_invoke_bind(wrapped, input, &output), 0);
      check_str_eq(turbo_runtime_data_bind_value_as_string(
                       turbo_runtime_data_bind_object_get(output, "value")),
                   "wrapped");
      check_int_eq((int)turbo_runtime_data_bind_value_as_int64(
      ...
      check_true(turbo_runtime_data_bind_value_as_bool(
          turbo_runtime_data_bind_object_get(output, "after"), 0));
    ...
    it("should apply runnable wrapper hooks while streaming") {
      ...
      check_not_null(log);
      check_int_eq(
          turbo_runnable_invoke_bind_stream(wrapped, input, turbo_event_log_capture_bind, log,
                                            &output),
          0);
      check_not_null(output);
    ...
    it("should apply runnable wrapper hooks to each batch item") {
      ...
      check_int_eq(turbo_runnable_batch_bind(wrapped, inputs, &outputs), 0);
      check_size_eq(turbo_runtime_data_bind_value_size(outputs), 2);
      ...
      check_true(turbo_runtime_data_bind_value_as_bool(
          turbo_runtime_data_bind_object_get(
              turbo_runtime_data_bind_array_get(outputs, 0), "after"),
          0));
      check_true(turbo_runtime_data_bind_value_as_bool(
          turbo_runtime_data_bind_object_get(
              turbo_runtime_data_bind_array_get(outputs, 1), "after"),
          0));
    ...
    it("should wrap state graphs as runnables") {
      ...
      turbo_runtime_data_bind_value_t *default_value =
          turbo_runtime_data_bind_value_create_bool(0);
      turbo_runtime_data_bind_value_t *input = turbo_runtime_data_bind_value_create_object();
      turbo_runtime_data_bind_value_t *output = NULL;
      turbo_state_graph_channel_config_t channel = {
          .reducer = TURBO_STATE_GRAPH_REDUCER_REPLACE,
          .value_kind = TURBO_RUNTIME_DATA_BIND_VALUE_BOOL,
      ...
      check_int_eq(turbo_state_graph_add_bind_node(graph, "done", state_graph_mark_bind, NULL),
                   TURBO_STATE_GRAPH_OK);
      ...
      check_not_null(output);
      check_true(turbo_runtime_data_bind_value_as_bool(
          turbo_runtime_data_bind_object_get(output, "state_done"), 0));
