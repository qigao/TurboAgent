...
spec("runtime cancellation control") {
  it("creates an active token without a deadline") {
    ...
    check_int_eq(turbo_cancel_token_check(token), TURBO_OK);
  ...
  it("keeps the first cancellation reason") {
    ...
    check_int_eq(turbo_cancel_source_cancel(source, TURBO_CANCEL_SHUTDOWN), TURBO_EALREADY);
    check_int_eq(turbo_cancel_token_check(token), TURBO_ECANCELED);
  ...
  it("materializes an expired monotonic deadline") {
    ...
    check_int_eq(turbo_cancel_source_token(source, &token), TURBO_OK);
    check_int_eq(turbo_cancel_token_check(token), TURBO_ETIMEDOUT);
    ...
    check_int_eq(turbo_cancel_source_cancel(source, TURBO_CANCEL_USER), TURBO_EALREADY);
  ...
  it("wakes every waiter when shutdown is requested") {
    ...
    for (i = 0; i < WAITER_COUNT; ++i) {
      check_int_eq(turbo_cancel_source_token(source, &waiters[i].token), TURBO_OK);
      atomic_init(&waiters[i].entered, 0);
      check_int_eq(turbo_thread_create(&threads[i], cancel_waiter_run, &waiters[i]), TURBO_OK);
    }
    ...
    check_int_eq(turbo_cancel_source_cancel(source, TURBO_CANCEL_SHUTDOWN), TURBO_OK);
  ...
  it("rejects an incompatible configuration without publishing a handle") {
    ...
    check_int_eq(turbo_cancel_source_create(&config, &source), TURBO_EINVAL);
    check_null(source);
    check_int_eq(turbo_cancel_source_cancel(NULL, TURBO_CANCEL_USER), TURBO_EINVAL);
  }
}
