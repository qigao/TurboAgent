...
void demo_jws_signing() {
    ...
    // Some private claims
    jwt.private_claims = turbo_json_create_object();
    turbo_json_object_set_string(jwt.private_claims, "role", "admin");
    ...
    if (rv == CJWTE_OK) {
        ...
        if (rv == CJWTE_OK) {
            ...
            printf("Decoded Role: %s\n", turbo_json_get_string(decoded->private_claims, "role"));
