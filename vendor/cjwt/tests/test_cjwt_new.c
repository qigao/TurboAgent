...
static void claims_eq(__bdd_config_type__ *__bdd_config__, const expected_claim_t *exp, json_value_t *act)
{
    char *text = NULL;
...
static void json_test_case(__bdd_config_type__ *__bdd_config__, const json_test_case_t *t)
{
    ...
    if (CJWTE_OK == result) {
        ...
        printf("kid: %s\n", jwt->header.kid);
        bdd_invoke(claims_eq, &t->exp_private_header, jwt->header.private_headers);
        ...
        bdd_invoke(claims_eq, &t->exp_private_claim, jwt->private_claims);
