...
suite("cjwt jwks") {
  group("parse") {
    it("parses a JWKS with RSA and EC keys") {
      ...
      check_not_null(jwks);
      check_int_eq(2, jwks->count);

      check_int_eq(CJWT_KTY_RSA, jwks->keys[0]->kty);
      check_str_eq("rsa-key-1", jwks->keys[0]->kid);

      check_int_eq(CJWT_KTY_EC, jwks->keys[1]->kty);
      check_str_eq("ec-key-1", jwks->keys[1]->kid);
