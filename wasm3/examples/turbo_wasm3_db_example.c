...
#ifndef WASM3_DB_EXAMPLE_WASM_PATH
#error "WASM3_DB_EXAMPLE_WASM_PATH must be defined by CMake"
#endif
...
int main(void) {
  ...
  result = turbo_wasm3_vm_load_module_file(vm, WASM3_DB_EXAMPLE_WASM_PATH,
                                           "db_demo", &module);
  if (result) {
  ...
  result = m3_FindFunction(&run, turbo_wasm3_vm_get_runtime(vm), "run_db_demo");
  if (result) {
    rc = fail_result("find run_db_demo", result);
    goto done;
  }

  result = m3_CallV(run);
  if (result) {
    rc = fail_result("call run_db_demo", result);
    goto done;
  }

  result = m3_GetResultsV(run, &result_value);
  if (result) {
    rc = fail_result("read run_db_demo result", result);
    goto done;
  }

  printf("db demo result = %d\n", result_value);
  if (result_value != 7) {
    fprintf(stderr, "unexpected db demo result: %d\n", result_value);
    rc = 1;
  }
